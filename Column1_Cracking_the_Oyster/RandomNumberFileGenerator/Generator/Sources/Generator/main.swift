//
//  RandomNumberFileGenerator.swift
...
struct Generator: ParsableCommand {
    ...
    @Argument(help: "The number of digits")
    ...
    func run() throws {
        let integers: [Int] = uniqueRandomNumbers(min: 0, max: numberOfDigits, count: numberOfIntegers)
        let output = integers.map(String.init).joined(separator: "\n")
        let filePath = FileManager.default.currentDirectoryPath.appending("RandomIntegers-digits\(numberOfDigits)-integers\(numberOfIntegers).txt")
